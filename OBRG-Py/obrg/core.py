...
from .visualization import (draw_complete, draw_incomplete, draw_leaf_centers,
                            draw_unallocated, draw_boundaries, draw_planar_nplanar)
...
def extract_boundary_voxels(cluster: Set[Octree]) -> Set[Octree]:
    cluster_centers = set(tuple(np.around(voxel.center, decimals=6))
                          for voxel in cluster)
    boundaries = set([leaf for leaf in cluster
                      if get_neighbor_count_same_cluster(leaf, cluster_centers) < 8])
    return boundaries
...
def refinement(is_planar, oc, incomplete_segment, b_v, kdtree, debug=False):
    if is_planar:
        # fast refinement
        print('planar!')
...
# @bench_this
def calculate(cloud_path: str, output_path: str, debug=False, save=True):
    """
    ...
    start = time()
    # A1a voxelization + A1b saliency (done inside create)
    oc = Octree(points, center=bb.get_center(),
                normals=np.asarray(cloud.normals))
    oc.create(d=0.4, r_th=0.08, d_min=0.05)
    pre = time()-start
    if debug:
        print(f'Octree has {len(oc.leaves)} leaves')
    ...
    planar_segs, nplanar_segs = [], []
    for i, incomplete_segment in enumerate(tqdm(incomplete_segments)):
        ...
        if is_planar:
            planar_segs.append(incomplete_segment)
        else:
            nplanar_segs.append(incomplete_segment)

        # B3 Refinement (FR or GR)
        refinement(
            is_planar, oc, incomplete_segment, b_v, KDTree, debug)
        complete_segments.append(incomplete_segment.union(b_v))
    if debug:
        if planar_segs and nplanar_segs:
            draw_planar_nplanar(planar_segs, nplanar_segs)
